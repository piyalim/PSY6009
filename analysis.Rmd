...
setwd("C:/Users/piyal/OneDrive/Desktop/R/dissertation")
...
trial_data <- read_csv("data/dissertation_data.csv", col_types = cols(
...
asq_data <- read_csv("data/Piyali_data WITH CONDITION  - ASQ baseline.csv")


#Preparing ASQ dataset
asq_clean <- asq_data %>% 
...
check_item1 <- asq_clean[ , c("c_high_pitch", "comm_item1")]
head(check_item1)
...
check_gm_item1 <- asq_clean[ , c("gm_lift_legs", "gmotor_item1")]
head(check_gm_item1)
...
check_fm <- asq_clean[ , c("fm_pickup_onehand", "fmotor_item6")]
head(check_fm)
...
check_ps <- asq_clean[ , c("ps_pass", "ps_item5")]
head(check_ps)
...
check_persoc <- asq_clean[ , c("persoc_mirror_reach", "persoc_item4")]
head(check_persoc)
...
## adding the total asq scores to the trial_clean dataframe

trial_data <- trial_data %>% 
  left_join(select(asq_clean, participant, comm_total, gmotor_total, fmotor_total, ps_total, persoc_total),
            by = "participant")
...
trial_clean <- trial_data %>% 
  ...
  mutate(IMD = as.factor(IMD)) %>% 
    mutate(click_percent = click_number/total_clicks * 100) # calculating percentage of links clicked
...
hist(trial_clean$age_of_child)

##check if distribution is Poisson
poisson_model <- glm(cdi_18m ~ 1, family = poisson, data = trial_no_na)
...
#not giving an AIC value
m0 <- glm(cdi_18m ~ scale(age_of_child,scale=FALSE) + condition, family = quasipoisson, data = trial_clean)

#2nd try
m_0_alt <- glmer.nb(cdi_18m ~ scale(age_of_child,scale=FALSE) + condition, (1|participant), data = trial_clean)

#can we just assume it's poisson
m0 <- glm(cdi_18m ~ scale(age_of_child,scale=FALSE) + condition, family = poisson, data = trial_clean)

#checking correlation between IMD and caregiver education
cor.test(y = trial_clean$IMD, x = as.numeric(trial_clean$caregiver_education), method = "pearson")

#visualisation - ordered bar plot
ggplot(trial_clean, aes(x = reorder(caregiver_education,IMD),
                       y = IMD)) +
  geom_bar(stat = "identity") +
  labs(title = " Relationship between IMD and caregiver education",
       x = "Caregiver Education",
       y = "Deprivation Index")

#boxplot
ggplot(trial_clean, aes(x = caregiver_education,
                       y = IMD)) +
  geom_boxplot() +
  labs(title = " Relationship between IMD and caregiver education",
       x = "Caregiver Education",
...
```{r}
#Generate compliance score compliance_ps: THIS IS THE PROBLEM!

m_adherence <-  glm(click_percent ~ sex_of_child + age_of_child + comm_total + gmotor_total + fmotor_total + ps_total + persoc_total + IMD + offset(log(total_clicks)), family = quasipoisson, data=trial_clean[trial_clean$condition == 1,])

m_adherence_p <-  glm(click_percent ~ sex_of_child + age_of_child + comm_total + gmotor_total + fmotor_total + ps_total + persoc_total + IMD + offset(log(total_clicks)), family = poisson, data=trial_clean[trial_clean$condition == 1,])

m_adherence <-  glm(click_percent ~ sex_of_child + age_of_child + baseline_asq + IMD + offset(log(total_clicks)), family = poisson, data=trial_clean[trial_clean$condition == 1,])



#The continuous propensity score will then be generated for all participants in both arms of the trial

trial_no_na$adherence_ps=predict(m_adherence,trial_no_na,type="response")

#The next step will be to fit the model with compliance as a continuous variable

m_adherence_mod = glm(cdi_18m ~ condition*adherence_ps, family = poisson, data = trial_no_na)

m_adherence_mod_alt = glmer.nb(cdi_18m ~ condition*adherence_ps, (1|participant), data = trial_no_na)
...
### Prediction 2  
1. The effect of intervention on 18 month vocab will be greater for caregivers who had higher adherence, and this interaction will be moderated by caregiver education.

2. The m_moderation model with the interactions between all three terms will have the lowest AIC value.
