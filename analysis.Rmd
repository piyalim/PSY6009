...
##This first part is creating and cleaning the data. (You can run the entire 1st chunk)
...
m0_quasipoisson <- glm(cdi_18m ~ scale(age_of_child,scale=FALSE) + condition, family = quasipoisson, data = trial_clean)

#2nd try asks for random effects and then does not recognise the v"participant" variable
m0_nb <- glmer.nb(cdi_18m ~ scale(age_of_child,scale=FALSE) + condition, (1|participant), data = trial_clean)

# trying the model with poisson distribution
m0_poisson <- glm(cdi_18m ~ scale(age_of_child,scale=FALSE) + condition, family = poisson, data = trial_clean)
...
```{r}
#Generate compliance score compliance_ps:

##gives AIC value = NA when family = quasipoisson
m_adherence_qp <-  glm(click_percent ~ sex_of_child + age_of_child + comm_total + gmotor_total + fmotor_total + IMD + offset(log(total_clicks)), family = quasipoisson, data=trial_clean[trial_clean$condition == 1,])


#gives AIC value of infinity when family = poisson
m_adherence_p <-  glm(click_percent ~ sex_of_child + age_of_child + comm_total + gmotor_total + fmotor_total + IMD + offset(log(total_clicks)), family = poisson, data=trial_clean[trial_clean$condition == 1,])
